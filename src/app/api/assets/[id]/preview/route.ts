...
function safeFilename(value: string) {
  return value.replace(/[^a-zA-Z0-9.\-_ ]/g, "_").trim() || "preview";
}

function filenameFromStorageKey(storageKey: string) {
  const base = storageKey.split("/").pop() || "preview";
...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Keep the same read rules as download endpoint.
  ...
  const previewName = filenameFromStorageKey(row[0].storageKey);
  const command = new GetObjectCommand({
    Bucket: config.bucket,
    Key: row[0].storageKey,
    ResponseContentDisposition: `inline; filename="${previewName}"`,
